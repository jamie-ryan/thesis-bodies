...
RHESSI captured a wide range of high energy emission throughout the flare, see Figure \ref{rhessicft}, however, for this project, only the 10 to 100 keV HXR range are sampled. During the impulsive phase of a flare, high energy x-ray emission is an indicator that accelerated particles are present. Assuming that the chromosphere is a thick target \citep{1971SoPh...18..489B}, the deposition of energy by accelerated particles is due to collisions between charged particles and ions producing hard X-ray bremsstrahlung emission. Using equation \ref{pnth} one can calculate the power, $P$, injected into the atmosphere by non-thermal electrons.  
...
For the IRIS SJI, IRIS SG and SDO HMI data sets, multiple sample points have been chosen based on the moment in time when the IRIS SG slit is directly over the southern flare ribbon and sunquake impact location. These coordinates are sampled across all data sets except RHESSI which has limited spatial resolution. Figure \ref{mgwrib} shows the sample coordinates, RHESSI HXR and the 6mHz sunquake egression map, plotted over IRIS SJI MG II data in an effort to demonstrate the spatial alignment between HXR (particle beam), UV ribbons and sunquake impact. Table \ref{coordtab} shows how each sample number relates to a heliocentric position in arcseconds.
\centering
\begin{tabular}{|c|c|}[H]
...
6 & 526.252", 263.818"\\
\end{tabular}
\caption{Sampleoordinates and Energies at 17:45 and 17:46 for ribbon sample locations.}\label{coordtab}

\begin{figure}[H]
  \begin{center}
  \includegraphics[width=0.8\textwidth]{29-Mar-14-MG-Ribbon-Coord-oplot}
  \end{center}
  \caption{The image on the left shows IRIS Mg II slit-jaw data with contours representing HXR emission and 6mHz sunquake power. The image on the right shows IRIS Mg II slit-jaw data zoomed in on the southern ribbon, with sample coordinates (see table \ref{coordtab}) and 6mHz sunquake power contours. The six ribbon sample points relate to the IRIS SG slit position at 17:46. Each of the IRIS SJ and SDO HMI data sets are sampled at the exact same coordinates in heliocentric units.}\label{mgrib}
\end{figure}
